  ...
  switch (client.token_endpoint_auth_method) {
    ...
    case "client_secret_basic": {
      ...
      headers.set(
        ...
        clientSecretBasic(
          client.client_id,
          assertClientSecret(client.client_secret),
        ),
      );
      break;
    }
    ...
    default:
      throw new UnsupportedOperationError(
        "unsupported client token_endpoint_auth_method",
      );
  }
}
...
function validateEndpoint(value, endpoint, options) {
  if (typeof value !== "string") {
    if (options?.[useMtlsAlias]) {
      throw new TypeError(
        `"as.mtls_endpoint_aliases.${endpoint}" must be a string`,
      );
    }
...
export async function processPushedAuthorizationResponse(as, client, response) {
  ...
  if (!validateString(json.request_uri)) {
    throw new OPE(
      '"response" body "request_uri" property must be a non-empty string',
    );
  }
  ...
  if (!ignoreIdToken) {
    ...
    if (json.id_token) {
      const { claims } = await validateJwt(
        ...
        .then(
          validatePresence.bind(undefined, ["aud", "exp", "iat", "iss", "sub"]),
        )
...
function checkJwtType(expected, result) {
  if (
    ...
    normalizeTyp(result.header.typ) !== expected
  ) {
...
export async function validateDetachedSignatureResponse(
  ...
  if (claims.s_hash !== undefined && typeof expectedState !== "string") {
    throw new OPE(
      'could not verify ID Token "s_hash" (state hash) claim value',
    );
  }
  if (
    typeof expectedState === "string" &&
    (typeof claims.s_hash !== "string" ||
      (await idTokenHashMatches(
        expectedState,
        claims.s_hash,
        header.alg,
        key,
  ...
  if (
    client.require_auth_time !== undefined &&
...
export async function processDeviceAuthorizationResponse(as, client, response) {
  ...
  if (!validateString(json.device_code)) {
    throw new OPE(
      '"response" body "device_code" property must be a non-empty string',
    );
  }
  if (!validateString(json.user_code)) {
    throw new OPE(
      '"response" body "user_code" property must be a non-empty string',
    );
  }
  if (!validateString(json.verification_uri)) {
    throw new OPE(
      '"response" body "verification_uri" property must be a non-empty string',
    );
  }
  if (typeof json.expires_in !== "number" || json.expires_in <= 0) {
    throw new OPE(
  ...
  if (
    ...
    throw new OPE(
      '"response" body "verification_uri_complete" property must be a non-empty string',
    );
  }
  if (
    ...
    throw new OPE(
      '"response" body "interval" property must be a positive number',
    );
  }
  return json;
}
...
async function validateDPoP(
  ...
  if (
    request.headers.get("authorization")?.toLowerCase().startsWith("dpop ") ===
    false
  ) {
    throw new OPE(
  ...
  const proof = await validateJwt(
    ...
    async ({ jwk, alg }) => {
      ...
      if (key.type !== "public") {
        throw new OPE(
          "DPoP Proof jwk header parameter must contain a public key",
        );
      }
      return key;
    },
    ...
    .then(
      validatePresence.bind(undefined, ["iat", "jti", "ath", "htm", "htu"]),
    );
  const now = epochTime() + clockSkew;
  ...
  }
  {
    ...
    const expected = b64u(
      await crypto.subtle.digest(
        "SHA-256",
        encoder.encode(JSON.stringify(components)),
      ),
    );
...
export async function validateJwtAccessToken(
  ...
  switch (scheme) {
    ...
    default:
      throw new UnsupportedOperationError(
        "unsupported Authorization HTTP Header scheme",
      );
  }
  if (length !== 2) {
  ...
  if ("cnf" in claims) {
    ...
    if (length) {
      ...
      if (cnf !== "jkt") {
        throw new UnsupportedOperationError(
          "unsupported JWT Confirmation method",
        );
      }
    }
  }
